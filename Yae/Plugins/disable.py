...
from Yae.Handlers.managers import CMD_STARTERS, SpamChecker
from Yae.Handlers.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    from Yae.Database import disable_sql as sql
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "YaeRobot.modules." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "YaeRobot.modules." + args[0].rsplit(".", 1)[0]
