...
from Yae import INSPECTOR, dispatcher
...
from Yae.Handlers.extraction import (
    extract_user,
    extract_user_and_text,
)
from Yae.Plugins.Admin.log_channel import loggable
from Yae.Handlers.alternate import send_message, typing_action
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    bot.sendMessage(
        chat.id,
        f"Successfully Made Demon Lord <b>{user_member.user.first_name or user_id}</b>!",
...
def fullpromote(update: Update, context: CallbackContext) -> str:
    ...
    bot.sendMessage(
        chat.id,
        f"Sucessfully Made Dragon Lord <b>{user_member.user.first_name or user_id}</b>!",
        parse_mode=ParseMode.HTML,
    )
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        bot.sendMessage(
            chat.id,
            f"Sucessfully Made Peasent Again <b>{user_member.user.first_name or user_id}</b>!",
            parse_mode=ParseMode.HTML,
        )
        ...
        message.reply_text(
            "Could not demote, May be the person is too sexy to be demoted."
...
def adminlist(update, context):
    ...
    text = "Emperors Of <b>{}</b>:".format(html.escape(update.effective_chat.title))
