...
from Yae.Handlers.validation import (
    bot_admin,
    can_restrict,
    connection_status,
    is_user_admin,
...
from Yae.Handlers.extraction import extract_user_and_text
from Yae.Handlers.string_handling import extract_time
from Yae.Plugins.Admin.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            ...
            message.reply_text(
                "Bring an order from Yae Management to fight a Requester."
            )
        else:
    ...
    log = (
        ...
        f"#{'S' if silent else ''}Beheaded The Stupid Ass\n"
        f"<b>Beheader:</b> {mention_html(user.id, html.escape(user.first_name))}\n"
        f"<b>Dead Ass:</b> {mention_html(member.user.id, html.escape(member.user.first_name))}"
    )
    if reason:
...
def punch(update: Update, context: CallbackContext) -> str:
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"Kicked User! {mention_html(member.user.id, html.escape(member.user.first_name))} On the Balls.",
            parse_mode=ParseMode.HTML,
        )
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
...
def unban(update: Update, context: CallbackContext) -> str:
    ...
    message.reply_text("Yep, that stupid ass can join again.")
...
def selfunban(context: CallbackContext, update: Update) -> str:
    ...
    message.reply_text("Yep, Gave A Second Chance To Your Noobself.")
